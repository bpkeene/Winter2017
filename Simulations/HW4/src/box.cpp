...
std::vector<double> Box::computeDistanceVector(Atom &atom1, Atom &atom2) {
    ...
    if (dx > boxlenhalf) dxm =- boxlen;
    if (dx < -boxlenhalf) dxm =+ boxlen;
    if (dy > boxlenhalf) dym =- boxlen;
    if (dy < -boxlenhalf) dym =+ boxlen;
    if (dz > boxlenhalf) dzm =- boxlen;
    if (dz < -boxlenhalf) dzm =+ boxlen;
    ...
    // return the distance
    std::vector<double> ret = std::vector<double> (3, 0.0);
